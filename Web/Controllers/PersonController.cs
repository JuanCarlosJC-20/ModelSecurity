...
namespace Web.ContUserlers
{
    ...
    public class PersonController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetPersonById(int id)
        {
            ...
            {
                var Person
                    = await _PersonBusiness.GetPersonByIdAsync(id);
                return Ok(Person);
            }
            catch (ValidationException ex)
            {
                _logger.LogWarning(ex, "Validación fallida para el permiso con ID: {PersonlId}", id);
                return BadRequest(new { message = ex.Message });
            }
            catch (EntityNotFoundException ex)
            {
                _logger.LogInformation(ex, "Permiso no encontrado con ID: {PersonlId}", id);
